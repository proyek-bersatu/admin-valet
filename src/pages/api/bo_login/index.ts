...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Data>) {
    try {
        ...
        if (req.method === 'POST') {
            ...
            const result = await axios.post(CONFIG.base_url_api + '/v1/bo/auth/login', {
                email, password
            }, {
                headers: {
            ...
            console.log(result?.data?.data, 'lslsl');

            const token = result.data?.data?.token;

            const user = await axios.get(CONFIG.base_url_api + "/v1/bo/profile", {
                headers: {
