...
import PartnerCreateModal from "@/components/modals/partner/create";
import PartnerDeleteModal from "@/components/modals/partner/delete";
import PartnerUpdateModal from "@/components/modals/partner/update";
import { CONFIG } from "@/config";
import { ColumnArea } from "@/constants/column_area";
import { ColumnPartner } from "@/constants/column_partner";
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  ...
  try {
    ...
    const table = await axios.get(
      `${CONFIG.base_url_api}/v1/bo/partner?${params.toString()}`,
      {
        headers: {
...
export default function PartnerPage({ table }: any) {
    ...
    action: (
      <div key={index} className="flex gap-2">
        <Button
          ...
          type="button"
          onClick={() => {
            router.push(`/bo/main/partner/${item?.id}`);
          }}
        >
  ...
  return (
    <div>
      ...
      {modal?.key == "create" && (
        <PartnerCreateModal open={modal?.open} setOpen={setModal} />
      )}
      {modal?.key == "update" && (
        <PartnerUpdateModal
          open={modal?.open}
